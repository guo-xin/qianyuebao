<template>
  <div @click.stop=""  @touchstart.stop="" class="calendar" v-show="show"  transition="calendar" transition-mode="out-in" >
    <div class="calendar-tools">
      <i class="fa fa-angle-left float left" @click.stop="prev">←</i>
      <i class="fa fa-angle-right float right" @click.stop="next" >→</i>
      ...
      select:function(k1,k2,e){
        ...
        if(that.range){
          ...
          if(that.type=='date'){
            ...
            that.show=false;
            that.$dispatch('handle-date', that.value)
          }
        }
...
<style scope>
  ...
  .calendar .timer input{
    ...
    width: 100px;
    text-align: center;
    border:2px solid #efefef;/*px*/
  }
  .calendar .timer input:focus{
    border:2px solid #5e7a88;/*px*/
  }
  .calendar-button{
    text-align: center;
  }
